...
class Queue {
  ...
  compareQueues(q2) {
    ...
    let tempThis = new Queue()
    tempThis = this
    let tempQ2 = new Queue()
    tempQ2 = q2
    for(let i=0; i<tempThis.size(); i++){
      const thisData = this.dequeue()
      const q2Data = q2.dequeue()
      if(tempThis.dequeue() != tempQ2.dequeue()){
        return false
      }
    }
  }
  ...
  isPalindrome() {
    ...
    let tempThis = new Queue()
    tempThis = this
    let stackThis = new Stack()
    for(let i = 0; i<this.size(); i++){
      stackThis.push(tempThis.dequeue())
    }
    tempThis = this
    let sizeQ = this.size()
    for(let j = 0; j<sizeQ; j++){
      if(tempThis.dequeue() != stackThis.pop()){
        return false
      }
  }
  return true;
