 ...
 class ListNode {
  ...
  class SinglyLinkedList {
    ...
    insertAtFront(data) {
      ...
      //creates a temporary variable to save the original head
      this.head = new ListNode(data);
      //creates a new head 
      this.head.next = temp;
     ...
     */
    // (garbage collection will detect the unreferenced to head the get rid of it form memory)
    removeHead() {
      const temp = this.head;
      this.head = this.head.next;
      return temp.data;
      //reassigns the head to the second element
      //because there is not pointer to the original head
      //this essentially remove it from the linked list
    ...
    average() {
      let runner = this.head;
      let sum = runner.data;
      //initializes the sum to the first value in the linked list
      let count = 1;
      //initializes the count of elements in the linked list to 1
      while (runner.next != null){
        runner = runner.next;
        sum += runner.data;
        count++;
